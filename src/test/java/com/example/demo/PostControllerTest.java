...
@WebFluxTest(controllers = PostController.class, excludeAutoConfiguration = {
...
public class PostControllerTest {
    ...
    public void getAllPostsWithKeyword_shouldBeOk() {
        ...
        given(this.posts.findByTitleContains("first", pageRequest))
                .willReturn(Flux.just(Post.builder().id("1").title("my first post").content("content of my first post")
                        .createdDate(LocalDateTime.now()).status(Post.Status.PUBLISHED).build()));

        this.client.get().uri("/posts?q=first").exchange().expectStatus().isOk().expectBody().jsonPath("$[0].title")
                .isEqualTo("my first post").jsonPath("$[0].id").isEqualTo("1").jsonPath("$[0].content")
                .isEqualTo("content of my first post");
    ...
    public void getAllPostsWithoutKeyword_shouldBeOk() {
        ...
        this.client.get().uri("/posts").exchange().expectStatus().isOk().expectBody().jsonPath("$[0].title")
                .isEqualTo("my first post").jsonPath("$[0].id").isEqualTo("1").jsonPath("$[0].content")
                .isEqualTo("content of my first post");
    ...
    public void getAllPostsByKeyword_shouldBeOk() {
        ...
        List<Post> data3 = List.of(5, 15, 15).stream()// 3 posts will be created.
                .map(n -> Post.builder().id("" + n).title("my " + n + " blog post")
                        .content("content of my " + n + " blog post").status(Post.Status.PUBLISHED)
                        .createdDate(LocalDateTime.now()).build())
                .collect(toList());
        PageRequest pageRequest = PageRequest.of(0, 10, Sort.by(Sort.Direction.DESC, "createdDate"));
    ...
    public void getPostById_shouldBeOk() {
        ...
        this.client.get().uri("/posts/1").exchange().expectStatus().isOk().expectBody().jsonPath("$.title")
                .isEqualTo("my first post").jsonPath("$.id").isEqualTo("1").jsonPath("$.content")
                .isEqualTo("content of my first post");
    ...
    public void getPostByNonExistedId_shouldReturn404() {
        ...
        this.client.get().uri("/posts/1").exchange().expectStatus().isNotFound();
    ...
    public void updatePost_shouldBeOk() {
        Post post = Post.builder().id("1").title("my first post").content("content of my first post")
                .createdDate(LocalDateTime.now()).build();
        ...
        given(this.posts.save(post)).willReturn(Mono.just(Post.builder().id("1").title("updated title")
                .content("updated content").createdDate(LocalDateTime.now()).build()));

        this.client.put().uri("/posts/1").body(BodyInserters.fromValue(post)).exchange().expectStatus().isNoContent()
                .expectBody().isEmpty();
    ...
    public void updatePostStatus_shouldBeOk() {
        ...
        this.client.put().uri("/posts/1/status").body(BodyInserters.fromValue(new UpdateStatusRequest("PUBLISHED")))
    ...
    public void createPost_withInvalidData_shouldReturn422() {
        ...
        // .willReturn(Mono.just(Post.builder().id("1").title("my first
        // post").content("content of my first
        // post").createdDate(LocalDateTime.now()).build()));

        this.client.post().uri("/posts").body(BodyInserters.fromValue(formData)).exchange().expectStatus()
    ...
    public void createPost_shouldBeOk() {
        ...
        given(this.posts.save(any(Post.class))).willReturn(Mono.just(Post.builder().id("1").title("my first post")
                .content("content of my first post").createdDate(LocalDateTime.now()).build()));

        this.client.post().uri("/posts").body(BodyInserters.fromValue(formData)).exchange().expectHeader()
                .value("Location", CoreMatchers.containsString("/posts/1")).expectStatus().isCreated().expectBody()
                .isEmpty();
    ...
    public void getCommentsByPostId_shouldBeOk() {
        given(this.comments.findByPost(any(PostId.class))).willReturn(Flux.just(Comment.builder().id("comment-id-1")
                .post(new PostId("1")).content("comment of my first post").build()));

        this.client.get().uri("/posts/1/comments").exchange().expectStatus().isOk().expectBody()
                .consumeWith(result -> log.debug("RESPONSE::" + new String(result.getResponseBody())))
                .jsonPath("$.[0].id").isEqualTo("comment-id-1").jsonPath("$.[0].content")
                .isEqualTo("comment of my first post");
    ...
    public void getCommentsCountByPostId_shouldBeOk() {
        ...
        this.client.get().uri("/posts/1/comments/count").exchange().expectStatus().isOk().expectBody()
                .consumeWith(result -> log.debug("RESPONSE::" + new String(result.getResponseBody())))
    ...
    public void createCommentOfPost_shouldBeOk() {
        ...
        given(this.comments.save(any(Comment.class)))
                .willReturn(Mono.just(Comment.builder().id("comment-id-1").post(PostId.builder().id("1").build())
                        .content("content of my first post").createdDate(LocalDateTime.now()).build()));

        CommentForm form = CommentForm.builder().content("comment of my first post").build();
        this.client.post().uri("/posts/1/comments").body(BodyInserters.fromValue(form)).exchange().expectStatus()
